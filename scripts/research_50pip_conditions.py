#!/usr/bin/env python3
"""Find indicator conditions associated with 50+ pip moves on H4/D1."""
...
def add_features(df):
    ...
    return pd.DataFrame({
      "ema_stack_bull":(e20>e50)&(e50>e200),"ema_stack_bear":(e20<e50)&(e50<e200),
      "price_above_ema20":c>e20,"price_above_200":c>e200,
      "rsi14":rsi,"macd_hist":mh,"adx14":adx,"pdi":pdi,"mdi":mdi,
      "atr_pct":atr.rolling(100).rank(pct=True),"bbpos":bbpos,
      "body_ratio":(c-o).abs()/(h-l).replace(0,np.nan),
    })

def label(df,pair,tf):
    pip=PIP[pair]; out=pd.DataFrame(index=df.index)
    for direction in ("bull","bear"):
      for hor in HORIZONS[tf]:
        vals=[]
        for i in range(len(df)):
          if i+hor>=len(df): vals.append(np.nan); continue
          fut=df.iloc[i+1:i+hor+1]; e=df.at[i,"close"]
          move=(fut.high.max()-e)/pip if direction=="bull" else (e-fut.low.min())/pip
          vals.append(move>=50)
        out[f"{direction}_h{hor}"]=vals
    return out
...
def main():
    ...
    for tf in TFS:
      for pair in PAIRS:
        ...
        z=pd.concat([add_features(df),label(df,pair,tf)],axis=1); z["pair"]=pair; z["timeframe"]=tf; datasets.append(z)
    ...
    conds={
      ...
      "rsi_gt70":lambda x:x.rsi14>70,"macd_hist_pos":lambda x:x.macd_hist>0,
      "adx_gt25":lambda x:x.adx14>25,"pdi_gt_mdi":lambda x:x.pdi>x.mdi,"mdi_gt_pdi":lambda x:x.mdi>x.pdi,
        ...
        for hor in HORIZONS[tf]:
          ...
          for name,fn in conds.items():
            ...
            out.append({"timeframe":tf,"direction":direction,"horizon_bars":hor,"condition":name,
                        "samples":n,"base_rate":round(base,4),"hit_rate":round(rate,4),
                        "lift":round(rate/base,3) if base else np.nan})
    out=pd.DataFrame(out).sort_values(["timeframe","direction","horizon_bars","lift"],ascending=[True,True,True,False])
    Path("reports").mkdir(exist_ok=True)
    out.to_csv("reports/50pip_condition_summary.csv",index=False)
    data.to_csv("reports/50pip_feature_dataset.csv",index=False)
